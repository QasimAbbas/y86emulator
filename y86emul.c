...
void rmmovl() {
    ...
    if((tempByte->low >=0 && tempByte->low <= 7) && (tempByte->high >=0 && tempByte->high <= 7)){
        ...
        *(int *)(emem + reg[tempByte->high] + *num) = reg[tempByte->low];
...
void mrmovl() {
    ...
    if((tempByte->low >=0 && tempByte->low <= 7) && (tempByte->high >=0 && tempByte->high <= 7)){
        //printf("BEFORE THE REGISTER IS DEFINED\n");
        reg[tempByte->low] = *(int *)(emem + reg[tempByte->high] + *num);
        //printf("%x\n", reg[tempByte->high]);
        //printf("%x\n", reg[tempByte->low]);
    }else{
        //printf("NOT A VALID REGISTER\n");
...
void pushl() {
    ...
    printf("PUSHL: VAL got pushed: %x\n", reg[tempByte->low]);
    printf("PUSHL: VAL got saved: %x\n", *(int*)(emem+reg[4]));
    eip++;
...
void popl() {
    ...
    printf("POPL: LOW: %i\n", tempByte->low);
    printf("POPL: HIGH: %i\n", tempByte->high);
    printf("POPL: REG %i WITH OLD VAL: %x IS BEING POPPED FROM STACK, TOP of STACk=%x, ESP=%x\n", tempByte->low, reg[tempByte->low], *(int*)(emem+reg[4]), reg[4]);
    if((tempByte->high == 15) && (tempByte->low >= 0 && tempByte->low <= 7)){
        printf("This is a valid register\n");
        printf("POPL: ESP WAS %x\n", reg[4]);
    ...
    printf("POPL: VAL being popped: %x\n", reg[tempByte->low]);
    eip++;
